...
async function createWarehouse(request: APIRequestContext, name: string, code: string): Promise<string> {
...
test.describe('TC-WHM-001 warehouseman panel access', () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    ...
    try {
      ...
      const organizationId = await firstOrganizationId(admin)
      const warehouseId = await createWarehouse(admin, warehouseName, `INT-${stamp}`)
      const assignedId = await createWarehouseman(admin, assignedEmail, ASSIGNED_NAME, organizationId)
      await createWarehouseman(admin, unassignedEmail, UNASSIGNED_NAME, organizationId)
      await assignWarehouse(admin, assignedId, warehouseId)
    } finally {
      await admin.dispose()
    }
  })
  ...
  test('a stub route is gated exactly like the panel home', async ({ context, page }) => {
